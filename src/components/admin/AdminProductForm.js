...
const AdminProductForm = () => {
  ...
  const handleInputChange = (e) => {
    ...
    if (type === 'number') {
      ...
      if (value === '') {
        ...
        if (!isNaN(numValue) && numValue >= 0) {
          ...
          return; // Don't update state for invalid number values
        }
      }
    }
    ...
    if (name === 'price') {
      ...
      if (!formData.basePrice && processedValue) {
        setFormData(prev => ({
          ...prev,
          price: processedValue,
          basePrice: processedValue
        }));
      } else {
        setFormData(prev => ({
          ...prev,
    ...
    else if (name === 'basePrice' && parseFloat(processedValue) < parseFloat(formData.price)) {
      setFormData(prev => ({
        ...prev,
        basePrice: processedValue,
        price: processedValue // Update price if basePrice becomes lower
      }));
    }
    else {
      setFormData(prev => ({
  ...
  const handleVariantImageUpload = async (variantIndex, e) => {
    ...
    try {
      ...
      const uploadingToast = toast.info('Uploading image to Cloudinary...', { autoClose: false });
      
      // Use the uploadImage service (now with Cloudinary)
      const response = await uploadImage(file, 'products');
      
      // Dismiss uploading toast
      toast.dismiss(uploadingToast);
      
      // Extract URL from response (Cloudinary format)
      let imageUrl = '';
      let publicId = null;
      
      if (response.data.file && response.data.file.url) {
        // Standard Cloudinary response format
      ...
      // Check if this is a fallback data URL (not recommended for production)
      if (imageUrl.startsWith('data:')) {
        toast.warning('⚠️ Image uploaded using fallback mode. Please check backend connection.', {
          autoClose: 5000
        });
      }
      ...
      handleAddVariantImage(variantIndex, {
        url: imageUrl,
        alt: file.name,
        publicId: publicId // Store Cloudinary public ID for future deletion
      });
      ...
      if (error.message) {
        ...
        } else if (error.message.includes('not a valid image type')) {
          errorMessage = '❌ Invalid file type. Only JPEG, PNG, GIF, WebP, SVG allowed';
