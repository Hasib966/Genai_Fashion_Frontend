...
const Navbar = () => {
  ...
  // Calculate wishlist length - ensure wishlist is an array
  const getWishlistCount = () => {
    if (!wishlist) return 0;
    if (Array.isArray(wishlist)) return wishlist.length;
    if (wishlist.data && Array.isArray(wishlist.data)) return wishlist.data.length;
    if (wishlist.wishlist && Array.isArray(wishlist.wishlist)) return wishlist.wishlist.length;
    if (wishlist.items && Array.isArray(wishlist.items)) return wishlist.items.length;
    return 0;
  };
  ...
  useEffect(() => {
    function handleClickOutside(event) {
      ...
      if (isSearchOpen && !event.target.closest('.navbar-search-inline') && !event.target.closest('.search-toggle')) {
  ...
  const toggleSearch = () => {
    ...
    // Close other menus
    if (isUserMenuOpen) setIsUserMenuOpen(false);
    // On mobile, close the mobile menu when opening search
    if (newSearchState && window.innerWidth <= 767) {
      setIsMobileMenuOpen(false);
    }
  };
        ...
        <div className="navbar-container">
          ...
          <nav className={`navbar-menu ${isMobileMenuOpen ? 'is-active' : ''} ${isLoading ? 'loading' : ''}`}>
            ...
            <div className="navbar-actions">
              {/* Inline Search Box */}
              {isSearchOpen ? (
                <form className="navbar-search-inline" onSubmit={handleSearch}>
                  <input
                    ref={searchInputRef}
                    type="text"
