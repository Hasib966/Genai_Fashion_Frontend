...
const ProductGrid = ({ 
  ...
  const wishlistItems = propWishlistItems.length > 0 ? propWishlistItems : ensureArray(wishlist);
              ...
              <div className="product-grid">
                {products
                  ...
                  .map(product => {
                    ...
                    // Check if product is in wishlist (handle different data structures)
                    const isInWishlist = wishlistItems.some(item => {
                      const itemProductId = item._id || item.id || item.productId;
                      const itemProduct = item.product || item;
                      const itemProductIdFromProduct = itemProduct._id || itemProduct.id || itemProduct.productId;
                      return (itemProductId === productIdentifier) || (itemProductIdFromProduct === productIdentifier);
                    });
