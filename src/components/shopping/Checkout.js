      ...
      try {
        ...
        const normalizedItems = extractedItems.map(item => {
          ...
          return {
            ...
            size: typeof item.size === 'object'
  ? (typeof item.size.name === 'object'
      ? (item.size.name.value || item.size.name.label || '')
      : item.size.name)
  : (item.size || item.variant_size || (item.variant && item.variant.size) || '')
    ...
    try {
      ...
      const productDetailsPromises = validCartItems.map(async (item) => {
        ...
        try {
          ...
          // Get size information and normalize to string
          let sizeName = (() => {
            const s = item.size;
            if (typeof s === 'string') return s;
            if (s && typeof s === 'object') {
              if (typeof s.name === 'string') return s.name;
              if (typeof s.name === 'object') return s.name?.value || s.name?.label || '';
              return s.value || s.label || s.code || '';
            }
            return item.sizeName || '';
          })();

          // Fallback if sizeName is empty: use first available size for selected color, or default "M"
          if (!sizeName || (typeof sizeName === 'string' && sizeName.trim() === '')) {
            const firstFromColor = (colorVariant?.sizes && Array.isArray(colorVariant.sizes) && colorVariant.sizes[0]) || null;
            const firstFromProduct = (productData?.colorVariants && productData.colorVariants[0]?.sizes && productData.colorVariants[0].sizes[0]) || null;
            const fallbackCandidate = firstFromColor || firstFromProduct || 'M';
            sizeName = typeof fallbackCandidate === 'object' ? (fallbackCandidate.name || 'M') : fallbackCandidate;
          }
      ...
      const orderData = {
        items: enrichedItems.map(item => ({
          ...
          size: {
            name: (typeof item.size?.name === 'string') ? item.size.name : (
              typeof item.size === 'string' ? item.size : (
                typeof item.size?.name === 'object' ? (item.size.name?.value || item.size.name?.label || '') :
                item.sizeName || ''
              )
            ),
            quantity: item.size?.quantity || item.quantity || 1
          }
        })),
        shippingAddress: {
          street: formData.address,
      ...
      // Debug final payload before API call
      try {
